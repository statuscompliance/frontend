...
import { getDatasourceById, updateDatasource, getDatasourceTypes, testDatasourceConnection } from '@/services/datasources';
...
export function DatasourceEdit() {
  ...
  // Cargar los tipos de datasource y el datasource específico al montar el componente
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Actualizar estado de conexión basado en las pruebas
      const updatedData = {
        ...formData,
        connectionStatus: connectionStatus === 'success' ? 'connected' : 
          connectionStatus === 'error' ? 'error' : 'unknown',
        lastSyncTime: connectionStatus === 'success' ? new Date().toISOString() : undefined
      };
      
      await updateDatasource(id, updatedData);
  ...
  if (initialLoading) {
    return (
      <Page name="Edit Data Source" className="h-full w-full">
      ...
      <div className="mx-auto max-w-3xl">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Update the configuration for this datasource
            </CardDescription>
          </CardHeader>
