...
import { createLinker, testLinkerConnection } from '@/services/linkers';
...
export function LinkerCreate() {
  ...
  useEffect(() => {
    async function fetchDatasources() {
      try {
        ...
        setDatasources(response.filter(ds => ds.status === 'active'));
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      await createLinker(formData);
      toast.success('Linker created successfully');
      navigate('/app/linkers');
    } catch (err) {
      toast.error('Failed to create linker');
  ...
  return (
    <Page name="Create Linker" className="h-full w-full">
      <div className="mx-auto max-w-4xl">
        ...
        <Card>
          <CardHeader>
            <CardTitle>Create New Linker</CardTitle>
            <CardDescription>
              Link datasources together to enable data flow between systems
            </CardDescription>
          </CardHeader>
